...
protected:
   ...
   bool Fill(const JsonObject &root)
   {
      ...
      feels_like        = root["current"]["feels_like"].as<float>();
      pressure          = root["current"]["pressure"].as<int>();
      humidity          = root["current"]["humidity"].as<int>();
      dew_point         = root["current"]["dew_point"].as<float>();
      uvi               = root["current"]["uvi"].as<int>();

      JsonArray hourly_list = root["hourly"];
      hourlyTime[0]    = LocalTime(root["current"]["dt"].as<int>());
      hourlyMaxTemp[0] = root["current"]["temp"].as<float>();
      hourlyMain[0]    = root["current"]["weather"][0]["main"].as<String>();
      ...
      hourlyIcon[0]    = root["current"]["weather"][0]["icon"].as<String>();
      for (int i = 1; i < MAX_HOURLY; i++) {
         if (i < hourly_list.size()) {
            ...
            hourlyMain[i]    = hourly_list[i - 1]["weather"][0]["main"].as<String>();
            ...
            hourlyIcon[i]    = hourly_list[i - 1]["weather"][0]["icon"].as<String>();
      ...
      for (int i = 0; i < MAX_FORECAST; i++) {
         if (i < dayly_list.size()) {
            ...
            forecastIcon[i]     = dayly_list[i]["weather"][0]["icon"].as<String>();
         }
